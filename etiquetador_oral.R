...
for (.f in c("stats_utils.R", "agreement.R", "corpus_stats.R", "plot_export.R", "prefs.R", "contexto.R", "i18n_util.R", "textgrid_parse.R")) {
...
server <- function(input, output, session) {
    ...
    if (is.null(rv$current_filename)) {
      ...
      tagList(
        ...
        fileInput("new_audio_upload",
                  i18n$t("Subir audio (si no está en www/audios):"),
                  accept = c(".wav", ".mp3"),
                  width  = "100%"),
  ...
  process_pending_tg <- function(tg_path, base, audio_path) {
    ...
    tg <- tryCatch(read_textgrid(tg_path), error = function(e) NULL)
    if (is.null(tg)) {
  ...
  observeEvent(input$regen_from_tg, {
    ...
    base       <- rv$pending_tg_base
    audio_path <- find_audio_for_base(base)
    tg <- tryCatch(read_textgrid(rv$pending_tg_path), error = function(e) NULL)
  ...
  # (read_textgrid, is_mfa_textgrid, build_ips_from_words_tier y
  #  parse_textgrid viven en R/textgrid_parse.R, compartido con los tests)
  # ============================================================
